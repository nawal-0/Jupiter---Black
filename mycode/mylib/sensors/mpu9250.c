...
extern void process_mpu9250(const struct device *dev, double *buffer)
{
    ...
    if (rc == 0)
    {
        ...
        buffer[0] = sensor_value_to_double(&accel[0]);
        buffer[1] = sensor_value_to_double(&accel[1]);
        buffer[2] = sensor_value_to_double(&accel[2]);
        buffer[3] = sensor_value_to_double(&gyro[3]);
        buffer[4] = sensor_value_to_double(&gyro[4]);
        buffer[5] = sensor_value_to_double(&gyro[5]);
    }
    else
    {
